...
export type AdminAnalyticsResponse = {
  ...
  referrers: Array<{ referrerHost: string; sampleReferrer: string | null; views: number; visitors: number }>;
...
export const api = {
  ...
  adminSendMarketing: (b: { campaignKey: string; userId?: number; userIds?: number[]; test?: boolean }) =>
    req<{ ok: boolean; status: string; jobId?: string; previousSentAt?: string | null; total?: number; sent?: number; duplicate?: number; skipped?: number; suppressed?: number; error?: number; results?: any[] }>("/admin/marketing/send", {
      method: "POST",
      body: JSON.stringify(b),
      timeoutMs: 30_000,
    }),
  adminMarketingJob: (id: string) => get<{ job: AdminMarketingJob }>(`/admin/marketing/jobs/${encodeURIComponent(id)}`),
  adminAnchors: (q = "") =>
